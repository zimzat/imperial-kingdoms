...
// } ----------------------------------------------------------------------------- {
		...
		function save()
		{
			$savable_data = array('fleet', 'planet', 'player', 'round');
			
			foreach ($savable_data as $save)
			{
				$this->save_items($save . 's', $save . '_id');
				
				if (!empty($this->data_old[$save . 's']) || !empty($this->data[$save . 's']))
				{
					$this->data_old[$save . 's'] = $this->data[$save . 's'];
				}
			}
		}
...
// } ----------------------------------------------------------------------------- {
		...
		function &round()
		{
			$return = &$this->item('rounds', 'round_id', $_SESSION['round']);
		...
		function &fleet($fleet_id)
		{
			$return = &$this->item('fleets', 'fleet_id', $fleet_id);
...
// } ----------------------------------------------------------------------------- {
		...
		function save_items($name_item, $name_id, $type = '')
		{
			if (empty($type))
			{
