...
Zapatec.Tree = function(el, config) {
		...
		param_default('selectOnExpand', true);
		param_default('compact', false);
		param_default('dynamic', false);
		param_default('initLevel', false);
		param_default('expandOnLabel', true);//expand/collapse the tree when the text or image are clicked
		param_default('saveState', false);//Keep track, using cookies, of the last location the user opened
		if (config.dynamic)
				config.initLevel = 0;
		this.config = config;
		// <PROFILE>
  ...
  if(this.config.saveState && Zapatec.Tree.prefs.state) {
				//restore to previous node
				this.sync(Zapatec.Tree.prefs.state);
		}
		// <PROFILE>
		if (this.config.d_profile) {
...
Zapatec.Tree.prototype.itemClicked = function(item_id, expand) {
		var selectedItem = this.toggleItem(item_id, null, expand);
		if (!expand || this.config.selectOnExpand) {
...
Zapatec.Tree.prototype.toggleItem = function(item_id, state, expand) {
				...
				var stateDefined = true;
		if (item_id) {
				if ((this.config.hiliteSelectedNode && this.selectedItem) && ((typeof expand != "undefined" && !expand) || this.config.selectOnExpand))
				...
				if ((expand || this.config.expandOnLabel || stateDefined) && (state != item.__msh_state)) {
...
Zapatec.Tree.onItemToggle = function() {
		...
		var expand = false;
		if((/tgb minus/.test(this.className)) || (/tgb plus/.test(this.className))) {
				expand = true;
		}
		...
		if(Zapatec.Tree.all[item.__msh_tree].config.saveState) {
				Zapatec.Tree.prefs.state = item.__msh_item;
				Zapatec.Tree.savePrefs();
		}
};
...
/// preferences
Zapatec.Tree.prefs = {
state	 : null  /**< Keeps track of which item the user choose*/
};
...
/**
 * \internal This function is used to load the current item of the tree from a cookie.
 * It tries to restore the tree to where it was in the previous page. This
 * is useful in trees of a web site to keep the tree open to the same place
 * from page to page.
 * Loads the cookie and merges saved prefs to Zapatec.Tree.prefs.
 */
Zapatec.Tree.loadPrefs = function() {
		var txt = Zapatec.Utils.getCookie("Zapatec_tree"), tmp;
		if (txt) {
...
/**
 * \internal Save the prefs of the tree to a cookie.
 */

Zapatec.Tree.savePrefs = function() {
		Zapatec.Utils.writeCookie("Zapatec_tree", Zapatec.Utils.makePref(this.prefs), null, '/', 7);
};


try {
		  Zapatec.Tree.loadPrefs();
} catch(e) {};
